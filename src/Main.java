...
public class Main {
    public static void main(String[] args) {//I change 1. and 4. employees Position(INTERN) and 2. and 3. employees startDate.ofYear(2023)
        List<Employee> processed_employees = readFileToList().stream()//2. Filter, Sort, and Modify Employee Data
                ...
                .toList();
        writeFileFromList(processed_employees, "src/processed_employees.txt");
        ...
        writeFileFromMap(department_summary, "src/department_summary.txt");
        ...
        List<Employee> new_employees = readFileToList().stream()//List employees who started in the last 2 years in new_employees.txt.
                ...
                .toList();
        writeFileFromList(new_employees, "src/new_employees.txt");


        Set<Employee> unique_employees = new HashSet<>(readFileToList());//5.  Identify Duplicate Employee  I add employee equals for solution
        writeFileFromList(unique_employees.stream().toList(), "src/unique_employees.txt");

        averageOfEachDepartment(department_summary, "src/average_salary_by_department.txt");


        Optional<Employee> min = readFileToList().stream()
                .min(Comparator.comparing(Employee::getStartDate));
        writeFileFromOptional(min, "src/longest_serving_employee.txt");//7. Find Longest-Serving Employee


        Map<Position, List<Employee>> employees_by_position = readFileToList().stream()//8. Count Employees by Position
                .collect(Collectors.groupingBy(Employee::getPosition));
        writeFileFromMapKeyPosition(employees_by_position, "src/employees_by_position.txt");


        writeFileFromMapKeyDepartmentThenPosition(department_summary, "src/employees_by_department_and_position.txt");
    }
    ...
    private static List<Employee> readFileToList() {//1. Read Employee Data from a Text File
        List<Employee> employeeList = new ArrayList<>();
        try (BufferedReader br = new BufferedReader(new FileReader("src/employees.txt"))) {
            String line;
    ...
    private static void writeFileFromList(List<Employee> processed_employees, String name) {//3. Write Processed Employee Data to a New Text File
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(name))) {
